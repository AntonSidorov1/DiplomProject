...
namespace MusicShopDesktopApp
{
    ...
    /// </summary>
    public class ProductsInShopsController 
    {
        /// <summary>
        /// Создать контроллер
        /// </summary>
            ...
            , int minDiscount = 0, int maxDiscount = 100, bool listFull = false)
        {
            ...
            if (id > 0)
            {
                if (!TradingPointsController.GetController().GetTradingPoints(session).GetShops().ContainsByID(id))
                {
                    throw new Exception("Торговый пункт не существует");
                }
            }
            ...
            return ProductsInPounktList.GetListFromDB(byName, byPrice, category, nameFilter, GetPounktType(), id,
                listFull).GetListByDiscount(minDiscount, maxDiscount);
        }
